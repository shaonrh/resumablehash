...
class TestSHA512:
    def test_basic_hash(self):
        h1 = resumablehash.sha512(b"hello world")
        h2 = hashlib.sha512(b"hello world")
        assert h1.digest() == h2.digest()
        assert h1.hexdigest() == h2.hexdigest()

    def test_constructor_with_data(self):
        h1 = resumablehash.sha512(b"test data")
        h2 = resumablehash.sha512()
        h2.update(b"test data")
        assert h1.hexdigest() == h2.hexdigest()

    def test_hexdigest_length(self):
        h = resumablehash.sha512(b"hello world")
    ...
    def test_copy_independence(self):
        h1 = resumablehash.sha512(b"hello")
        h2 = h1.copy()
        assert h1.hexdigest() == h2.hexdigest()
        h1.update(b" world")
        h2.update(b" there")
    ...
    def test_type_error_on_string(self):
        with pytest.raises(TypeError):
            resumablehash.sha512("not bytes")

    def test_empty_input(self):
        h1 = resumablehash.sha512(b"")
        h2 = hashlib.sha512(b"")
        assert h1.hexdigest() == h2.hexdigest()

    def test_empty_update(self):
        h1 = resumablehash.sha512(b"data")
        h2 = resumablehash.sha512(b"data")
        h2.update(b"")
        assert h1.hexdigest() == h2.hexdigest()

    def test_random_cross_validation(self):
        """200 random chunks with pickle roundtrip each iteration."""
        h1 = resumablehash.sha512()
        h2 = hashlib.sha512()
    ...
    def test_block_boundary_sizes(self):
        for size in [0, 1, 127, 128, 129, 255, 256, 1024]:
            ...
            data = data[:size]
            h1 = resumablehash.sha512(data)
            h2 = hashlib.sha512(data)
    ...
    def test_update_accepts_bytearray(self):
        ...
        h2 = resumablehash.sha512(b"hello")
        assert h1.hexdigest() == h2.hexdigest()

    def test_new_factory(self):
        h1 = resumablehash.new("sha512", b"hello")
        h2 = resumablehash.sha512(b"hello")
        assert h1.hexdigest() == h2.hexdigest()
